...
use app\admin\model\LoginLogs as MLoginLogs;
...
class User extends Base
{
     ...
     * @throws \think\exception\DbException
     */
    public function user_list($username = '')
    {
        $mUser = new MUsers();
        if ($username != '') {
        ...
        $finalRes = $mUser->formatUsersOutput($result, true);
        $page = $result->render();
        $this->assign('data', $finalRes);
        $this->assign('page', $page);
        return $this->fetch();
    ...
    {
        $data = ['pid' => $pid, 'uid' => $uid, 'createTime' => currentMillis()];
    ...
    {
        $loginLogs = new MLoginLogs();
        if($userId){
            $filter = [ 'uid' => trim($userId) ];
            $result = Db::name($loginLogs->tableName)->where($filter)->select();
            $page = null;
        }else{
            $result = Db::name($loginLogs->tableName)->where('uid','neq', '')->paginate(Config::get('myConfig.page_num'), false, ['query' => $_GET]);
            $page = $result->render();
        }
        if($userId != ''){
            $this->assign('quick', '<input type="button" name="Submit" class="btn btn-default radius" onClick="javascript:window.history.back();return false;" value="返回">');
        }
        $finalRes = $loginLogs->formatLoginLogsOutput($result);
        $this->assign('data', $finalRes);
        $this->assign('page', $page);
        return $this->fetch();
    }
    ...
    public function user_add_save( $name = '', $nickName = '',$password = '', $locked = 0)
    {
        ...
        $data = ['_id' => $uniqueId, 'name' => $name, 'nickName' => $nickName, 'avatar' => $avatar, 'password' => md5($password), 'locked' => ($locked ==1) ? true : false, 'createTime' => currentMillis()];
        $data['type'] = 'HUMAN';
        $validate = Loader::validate('User');
